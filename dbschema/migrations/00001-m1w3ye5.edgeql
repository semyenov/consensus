CREATE MIGRATION m1w3ye5c5kes4futtaq5zzmirewzplxaskxzn7yakpvfousdp2cyha
    ONTO initial
{
  ...
  CREATE TYPE user::User EXTENDING default::BaseObject, default::Auditable {
      CREATE REQUIRED LINK identity: ext::auth::Identity;
      CREATE REQUIRED PROPERTY email: std::str {
          CREATE CONSTRAINT std::exclusive;
      };
      CREATE ACCESS POLICY user_can_select
          ALLOW SELECT USING ((.identity ?= GLOBAL ext::auth::ClientTokenIdentity));
      CREATE INDEX ON (.identity);
      CREATE INDEX ON (.email);
  };
  CREATE GLOBAL user::current_user := (std::assert_single((SELECT
      user::User {
          id,
          name
      }
  ...
  CREATE TYPE finance::Budget EXTENDING default::BaseObject, default::Auditable, finance::Monetary {
      ...
      CREATE ACCESS POLICY budget_owner_can_select
          ALLOW SELECT USING ((.owner ?= GLOBAL user::current_user));
  ...
  CREATE TYPE issue::Issue EXTENDING default::BaseObject, default::Auditable, tag::Taggable, search::VectorSearchable {
      CREATE REQUIRED LINK author: user::User {
          SET default := (GLOBAL user::current_user);
      };
      ...
      CREATE ACCESS POLICY author_has_full_access
          ALLOW ALL USING ((.author ?= GLOBAL user::current_user));
      CREATE OPTIONAL LINK assignee: user::User;
      CREATE ACCESS POLICY issue_assignee_can_edit
          ALLOW UPDATE USING ((.assignee ?= GLOBAL user::current_user));
  ...
  CREATE TYPE project::Milestone EXTENDING default::Auditable, default::BaseObject, search::VectorSearchable {
      ...
      CREATE ACCESS POLICY milestone_budget_owner
          ALLOW ALL USING ((.budget.owner ?= GLOBAL user::current_user));
  ...
  CREATE TYPE project::Project EXTENDING default::Auditable, default::BaseObject, tag::Taggable, search::VectorSearchable {
      CREATE REQUIRED LINK budget: finance::Budget;
      CREATE MULTI LINK issues: issue::Issue {
          ON SOURCE DELETE DELETE TARGET;
      };
      ...
      CREATE ACCESS POLICY project_owner_has_full_access
          ALLOW ALL USING ((.owner ?= GLOBAL user::current_user));
  ...
  ALTER TYPE project::Milestone {
      ...
      CREATE ACCESS POLICY project_owner_has_full_access
          ALLOW ALL USING ((.project.owner ?= GLOBAL user::current_user));
  ...
  CREATE TYPE finance::Expense EXTENDING default::BaseObject, default::Auditable, finance::Monetary {
      CREATE REQUIRED LINK created_by: user::User;
      CREATE ACCESS POLICY expense_creator_can_select
          ALLOW SELECT USING ((.created_by ?= GLOBAL user::current_user));
      CREATE REQUIRED LINK budget: finance::Budget;
  };
  ...
  CREATE TYPE finance::Payment EXTENDING default::BaseObject, default::Auditable, finance::Monetary {
      CREATE REQUIRED LINK approved_by: user::User;
      CREATE ACCESS POLICY payment_approver_can_select
          ALLOW SELECT USING ((.approved_by ?= GLOBAL user::current_user));
      CREATE REQUIRED LINK associated_issue: issue::Issue;
      CREATE REQUIRED LINK recipient: user::User;
  ...
  ALTER TYPE issue::Issue {
      CREATE ACCESS POLICY project_members_can_edit
          ALLOW SELECT, UPDATE USING ((.milestone.project.owner ?= GLOBAL user::current_user));
  };
  CREATE TYPE project::Team EXTENDING default::BaseObject {
      ...
      CREATE MULTI LINK members: user::User;
      CREATE PROPERTY team_size := (std::count(.members));
  };
  ALTER TYPE user::User {
      CREATE ACCESS POLICY project_owner_can_select
          ALLOW SELECT USING ((.<owner[IS project::Project] ?= GLOBAL user::current_user));
  ...
  CREATE TYPE issue::Comment EXTENDING default::Auditable, search::VectorSearchable {
      CREATE REQUIRED LINK author: user::User;
  ...
  CREATE TYPE issue::Label EXTENDING default::Auditable, default::BaseObject, search::VectorSearchable {
