...
export function array(arg: any) {
  if (Array.isArray(arg)) {
    const items = arg.map((a) => {
      return literalToTypeSet(a)
    })
...
const tupleProxyHandlers: ProxyHandler<ExpressionRoot> = {
  get(target: ExpressionRoot, prop: string | symbol, proxy: any) {
    ...
    return items && Object.hasOwn(items, prop)
...
export function tuple(input: any) {
  if (Array.isArray(input)) {
    // is tuple
    if (input.every((item) => {
      return typeKinds.has(item.__kind__)
    })) {
 ...
 return item.__name__
    ...
    const items = input.map((item) => {
      return literalToTypeSet(item)
    })
    const name = `tuple<${items
      .map((item) => {
 return item.__element__.__name__
})
      .join(', ')}>`
    ...
    return $expressionify({
      ...
      __element__: makeTupleType(
        name,
        items.map((item) => {
          return item.__element__
        }),
      ),
      __cardinality__: cardutil.multiplyCardinalitiesVariadic(
        items.map((i) => {
          return i.__cardinality__
        }) as any,
      ),
      __items__: items,
    }) as any
  ...
  if (Object.values(input)
    .every((el: any) => {
      return typeKinds.has(el.__kind__)
    })) {
    const typeName = `tuple<${Object.entries(input)
        .map(([key, val]: [string, any]) => {
 return `${key}: ${val.__name__}`
})
        .join(', ')}>`
  ...
  const name = `tuple<${Object.entries(exprShape)
      .map(([key, val]) => {
 return `${key}: ${val.__element__.__name__}`
})
      .join(', ')}>`
  ...
  return $expressionify({
    ...
    __cardinality__: cardutil.multiplyCardinalitiesVariadic(
      Object.values(exprShape)
        .map((val) => {
          return val.__cardinality__
        }) as any,
    ),
    __shape__: exprShape,
  }) as any
...
export function $objectTypeToTupleType(...args: any[]): any {
  ...
  const shape = Object.entries(objExpr.__element__.__pointers__)
    .reduce(
      (_shape, [key, val]) => {
        if (
          fields?.length
