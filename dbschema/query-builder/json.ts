...
function jsonStringify(type: ParamType, val: any): string {
  if (type.__kind__ === TypeKind.array) {
    if (Array.isArray(val)) {
      return `[${val
        .map((item) => {
 return jsonStringify(type.__element__, item)
})
        .join()}]`
    }
  ...
  if (type.__kind__ === TypeKind.tuple) {
    if (!Array.isArray(val)) {
      throw new TypeError(`Param with tuple type is not an array`)
    }
    ...
    return `[${val
      .map((item, i) => {
 return jsonStringify(type.__items__[i]!, item)
})
      .join()}]`
  }
  if (type.__kind__ === TypeKind.namedtuple) {
    if (typeof val !== 'object') {
      throw new TypeError(`Param with named tuple type is not an object`)
    }
