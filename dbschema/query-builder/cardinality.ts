...
export namespace cardutil {
  ...
  >(cards: Cards): multiplyCardinalitiesVariadic<Cards> {
    ...
    if (cards.length === 1) {
      return cards[0] as any
    }

    return cards.reduce(
      (product, card) => {
        return multiplyCardinalities(product, card)
      },
      Cardinality.One,
    ) as any
  }
  ...
  >(cards: Cards): mergeCardinalitiesVariadic<Cards> {
    ...
    if (cards.length === 2) {
      return mergeCardinalities(first, second) as any
    }
  ...
  export function coalesceCardinalities(
    ...
    if (c1 === Cardinality.One || c1 === Cardinality.AtLeastOne) {
      return c1
    }
    if (c2 === Cardinality.One) {
      return overrideLowerBound(c1, 'One')
    }
    if (c2 === Cardinality.AtLeastOne) {
      return Cardinality.AtLeastOne
    }
