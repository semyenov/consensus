...
function makeGroupingSet(prefix: string) {
  return <T extends SimpleGroupElements>(grps: T): { [k in keyof T]?: T[k] } => {
    const seenKeys = new Map<string, SingletonSet>()
    const unfiltered = Object.entries(grps as NestedGroupElements)
      .flatMap(
        ([k, grp]) => {
          return (isGroupingSet(grp)
            ? grp.__exprs__
