  ...
  for (const [expr, refData] of seen) {
    ...
    if (refData.linkProps.length) {
      linkProps.set(
        ...
        refData.linkProps.map((linkProp) => {
          return linkProp.__parent__.linkName.slice(1)
        },
        ),
      )
    }
    ...
    if (
      ...
      const scopeVarName = `__scope_${
        withVars.size
      }_${scopeVar.__element__.__name__.replace(/[^a-z]/gi, '')}`
    ...
    if (
      ...
      if (!withBlock) {
        ...
        if (
          parentScopes.some(
            (parentScope) => {
              return parentScope && seen.has(parentScope)
            },
          )
        ) {
        ...
        const commonParentScope = firstParentScopeChain
          ...
          {
            return parentScopeChains.every((otherScope) => {
              return otherScope.includes(scope)
            },
            )
          },
          )
      ...
      for (const scope of [
        ...
        ...util.flatMap(refData.aliases, (alias) => {
          return [...walkExprCtx.seen.get(alias)!.parentScopes]
        }),
      ]) {
...
function renderEdgeQL(
  ...
  function renderWithBlockExpr(
    ...
    if (ctx.linkProps.has(expr)) {
      renderedExpr = `(SELECT ${renderedExpr} {\n${ctx.linkProps
        ...
        .map(
          (linkPropName) => {
 return `  __linkprop_${linkPropName} := ${renderedExprNoDetached}@${linkPropName}`
},
        )
        .join(',\n')}\n})`
    }
  ...
  if (ctx.withBlocks.has(expr as any) || scopeExpr) {
    ...
    if (!scopeExpr) {
      ...
      unscopedWithBlock.push(
        ...sortedBlockVars.map((blockVar) => {
          return renderWithBlockExpr(blockVar)
        }),
      )
    }
    ...
    // }
      ...
      const scopedVars = sortedBlockVars.filter((blockVarExpr) => {
        return ctx.withVars.get(blockVarExpr)?.childExprs.has(scopeExpr)
      },
      )
      ...
      unscopedWithBlock.push(
        ...sortedBlockVars
          .filter((blockVar) => {
            return !scopedVars.includes(blockVar)
          })
          .map((blockVar) => {
            return renderWithBlockExpr(blockVar)
          }),
 ...
 return renderWithBlockExpr(blockVar)
            ...
            .map((blockVar) => {
              const { name } = (ctx.withVars.get(blockVar)!)
...
return `  __param__${param.__name__} := ${paramExpr}`
    ...
    if (
      exprs.every((ex) => {
        return ex.__element__.__kind__ === TypeKind.object
      })
      || exprs.every((ex) => {
        return ex.__element__.__kind__ !== TypeKind.object
      })
...
.join(', ')} }`
    ...
    throw new Error(
        `Invalid arguments to set constructor: ${exprs
          .map((ex) => {
 return ex.__element__.__name__
})
          .join(', ')}`,
    )
  }
 ...
 return renderEdgeQL(item, ctx)
    ...
    return `(\n${expr.__items__
      .map(
        (item) => {
 return `  ${renderEdgeQL(item, ctx, renderShape, noImplicitDetached)}`
},
      )
 ...
 return `  ${key} := ${renderEdgeQL(
    ...
    return `(${renderEdgeQL(expr.__expr__, ctx, false, true)
.slice(
      1,
      -1,
    )}${clause.join('')})`
    ...
    const combinedBlock = [
      ...
      ...groupingSet.__exprs__.map(
        ([k, v]) => {
          return `  ${k} := ${renderEdgeQL(v, ctx)}`
        },
      ),
    ]
    groupStatement.push(`USING\n${combinedBlock.join(',\n')}`)

    let by = renderGroupingSet(groupingSet)
      .trim()
...
${indent(groupStatement.join('\n'), 4)}
  ...
  key: {${groupingSet.__exprs__.map((e) => {
 return e[0]
})
.join(', ')}},
  grouping,
  elements: ${elementsShapeQuery}
    ...
    const args = expr.__args__.map(
      (arg) => {
        return `${renderEdgeQL(arg!, ctx, false)}`
      },
    )
...
function shapeToEdgeQL(
  ...
  const addLine = (line: string) => {
    return lines.push(`${keysOnly ? '' : '  '}${line}`)
  }
  ...
  for (const key in shape) {
    ...
    const expectedCardinality
      ...
      && Object.hasOwn(val, '__cardinality__')
    ...
    if (typeof val !== 'object') {
      throw new TypeError(`Invalid shape element at "${key}".`)
    }

    const valIsExpression = Object.hasOwn(
      val,
      '__kind__',
    )
...
function literalToEdgeQL(type: BaseType, val: any): string {
  ...
  if (typename === 'std::json') {
    ...
    if (isArrayType(type)) {
      stringRep = `[${val
        .map((el) => {
 return literalToEdgeQL(type.__element__ as any, el)
})
        .join(', ')}]`
    }
      ...
      stringRep = `( ${val
        .map((el, j) => {
 return literalToEdgeQL(type.__items__[j] as any, el)
})
    ...
    if (isNamedTupleType(type)) {
      stringRep = `( ${Object.entries(val)
.map(
        ([key, value]) => {
 return `${key} := ${literalToEdgeQL(type.__shape__[key]!, value)}`
},
      )} )`
      skipCast = true
    }
