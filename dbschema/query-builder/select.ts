...
export function $handleModifiers(
  ...
  const mods: NormalisedSelectModifiers = {
    singleton: Boolean(modifiers.filter_single),
  }
  ...
  if (modifiers.filter_single) {
    ...
    if (fs.__element__) {
      ...
      const exprs = Object.keys(fs)
        .map((key) => {
          const val = fs[key].__element__
            ? fs[key]
            : (literal as any)(
                (root.__element__ as any as ObjectType).__pointers__[key]!.target,
                fs[key],
              )
  ...
  if (modifiers.order_by) {
    ...
    mods.order_by = orderExprs.map((expr) => {
      return (typeof (expr as any).__element__ === 'undefined'
        ? expr
        : { expression: expr })
    },
    ) as any
